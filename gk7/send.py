...
from util.page import HTML
from util.util import RandomUtil
from util.decrypt import decrypt
from task.tasks import MailTask, DownloadTask
...
class Send:
    ...
    ebookId  : 豆瓣书籍ID
    bookTitle: 图书标题
    sendType:  推送类型,详见：globals.BOOK_TYPE
    version:   插件版本
    '''
    @aop.exec_out_time
    def execute(self, args):
        try:
            ...
            if str(version) != globals.__VERSION__:
            ...
            self.db.email_add(email_id, to_email, book_title, book_author)

            book_info = self.db.book_get(ebook_id, book_size)
            # 如果书籍已经存在
            if book_info:
                # 修改待发送邮件附件信息
                ...
                if attach_file:
                    if os.path.exists(attach_file):
                        self.db.email_update_attach_file(email_id, attach_file)
                        # 发送邮件
                        ...
                        if to_private_email:
                            ...
                            self.db.email_add_full(private_email_id, to_private_email, book_title, book_author, attach_file)
                        ...
                        # 文件不存在,删除文件
                        self.db.book_delete(book_info['id'])

            # 创建HTML
            # 源文件目录[绝对路径](格式：主目录/douban书籍ID/书籍大小)
            file_dir = '%s/%s/%s' %(globals.DATA_DIRS, ebook_id, str(book_size))
            page = HTML(book_title, book_author, file_dir)
            book_html_local_path, book_images_remote_path = page.create(send_type, data_posts)

            # 书籍封面远程路径
            book_cover_remote_path = globals.BOOK_COVER_URL.replace('{#id}', ebook_id).replace('{#type}', send_type == globals.BOOK_TYPE['gallery'] and globals.BOOK_TYPE['article'] or send_type)
            # 存储书籍信息
            book_id = RandomUtil.random32Str()
            self.db.book_add(book_id, ebook_id, book_title, book_subtitle, book_author, book_size, book_cover_remote_path)
            # 抓取书籍封面并存储本地
            book_cover_task = DownloadTask.get_image.apply_async((book_cover_remote_path, globals.BOOK_COVER_DIRS), )
            # 更新书籍封面路径
            self.db.book_update_cover(book_id, book_cover_task.get())

            # 书籍图片下载任务
            book_images_task = None
            if len(book_images_remote_path):
                # 存储书籍图片路径信息
                self.db.book_img_add(RandomUtil.random32Str(), book_id, book_images_remote_path)
                # celery异步任务下载书籍图片
                job = group(DownloadTask.get_image.s(url, file_dir) for url in book_images_remote_path)
                book_images_task = job.apply_async()

            # 将待转换的书籍html信息存储在数据库中
            convert_id = RandomUtil.random32Str()
            self.db.convert_add(convert_id, to_email, book_html_local_path)

            # 开启异步进程，转换书籍并发送邮件
            # 书籍输出目录[OUT_DATA_DIRS/douban书籍ID/大小/]
            out_file_dir = '%s/%s/%s' %(globals.OUT_DATA_DIRS, ebook_id, str(book_size))
