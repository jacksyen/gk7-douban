...
class TBL_GLOBAL:
    ...
    def check_init(self):
        ...
        if result == None:
            self.db.execute('INSERT INTO %s(smtp, smtp_port, email_user, email_pwd, email_encode, addtime, updatetime) VALUES("%s", "%s", "%s", "%s", "%s", "%s", "%s")' %(Global.GLOBAL_EMAIL_SMTP, Global.GLOBAL_EMAIL_SMTP_PORT, Global.GLOBAL_EMAIL_USER, Global.GLOBAL_EMAIL_PWD, Global.GLOBAL_EMAIL_ENCODE, DateUtil.getDate(format='%Y-%m-%d %H:%M:%S'), DateUtil.getDate(format='%Y-%m-%d %H:%M:%S')))
