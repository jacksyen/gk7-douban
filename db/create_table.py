...
class Create:
    ...
    def gk7_douban_books():
        return 'CREATE TABLE IF NOT EXISTS %s(book_id text PRIMARY KEY, book_number text, book_title text, book_subtitle text, book_author text, book_file_path text, book_size INTEGER, addtime datetime, updatetime datetime)' %(Global.GLOBAL_DB_TBL_BOOK_NAME)
    ...
    def gk7_douban_book_img():
        return 'CREATE TABLE IF NOT EXISTS %s(book_id text PRIMARY KEY, book_images_local_path text, book_images_remote_path text, addtime datetime, updatetime datetime)' %(Global.GLOBAL_DB_TBL_BOOK_IMG_NAME)
